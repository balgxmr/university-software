...
int main(void) {
  ...
  while (true && eleccion != CASES_AMOUNT) {
    ...
    case 2:
      ...
      while (getchar() != '\n')
        ; // Truco para borrar el salto de linea que obtenemos en el primer
          // scanf.
      ...
      for (int i = 0; i < total; i++) {
        ...
        nombres[largo - 1] = '\0'; // borra el ultimo coso, el salto de linea
                                   // invisible ese ya tu sabe xd
        ...
        while (fechaValida != 1) {
          ...
          if (dia[i][0] > 33 || dia[i][0] < 1 || mes[i][0] < 1 ||
              mes[i][0] > 12 || anio[i][0] < 1900 || anio[i][0] > ANIO_ACTUAL) {
            printf(
                "Una de sus fechas está incorrecta, corrijala nuevamente! \n");
            fechaValida = 0;
          } else {
            int xd = 1;
            while (true && xd == 1) {
              printf("Su fecha es: Dia: %d , Mes: %d , Año: %d\n", dia[i][0],
                     mes[i][0], anio[i][0]);
              printf("\nSi desea corregirla, introduzca 1, si desea continuar "
                     "presione 0. \n");
              scanf("%d", &xd);
              if (xd == 1) {
                fechaValida = 0;
