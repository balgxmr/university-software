...
3. Cajero automático
    a. El cajero automático lo puede utilizar el cliente y el empleado de la
sucursal b. El cliente deberá identificarse en la terminal antes de realizar
cualquier operación c. El empleado y el cliente pueden retirar dinero. d.
Además, podrá cambiar el pin, obtener los últimos movimientos y saldo, y
realizar pagos tanto con visa o master card. e. El empleado tiene la función de
reponer billetes en el cajero f. El cualquier reclamo o problema, el cliente
debe reportarlo a través del empleado. g. El técnico es el único que puede
realizar las operaciones de mantenimiento en caso de un daño. h. El empleado es
el encargado de reportar los daños del cajero al técnico. i. Existe un
formulario que el cliente debe llenar cada vez que realice un reclamo.
...
struct Empleado empleado1 = {"Jose Manuel", "8-999-1002", 1420, 1259.30};

struct Empleado empleado2 = {"Antonio Carmona", "4-812-1249", 2503, 3259.35};

struct Empleado empleado3 = {"Nicole Guerra", "6-35-351", 2506, 1003.35};
...
struct Cliente cliente1 = {"Richard Martinez", "E-50-29526", 0105, 2582.00};

struct Cliente cliente2 = {"David Martinez", "8-599-1592", 0153, 3593.02};

struct Cliente cliente3 = {"Goatnel Messi", "6-395-2592", 3993, 6043.15};

/* FIN CLIENTES */

void verSaldo(int usuario, int id_usuario, struct Empleado *empleado,
              struct Cliente *cliente);
void retirarDinero(int usuario, int id_usuario, float retiro,
                   struct Empleado *empleado, struct Cliente *cliente);
void cambiarPin(int usuario, int id_usuario, struct Empleado *empleado,
                struct Cliente *cliente);
...
int main() {
  ...
  while (looper) {
    ...
    switch (seleccion) {
    case 1: // Ver saldo
      verSaldo(usuario, id_usuario, &empleados[id_usuario],
               &clientes[id_usuario]);
      break;
    ...
    case 2: // Retirar dinero
      ...
      retirarDinero(usuario, id_usuario, retiro, &empleados[id_usuario],
                    &clientes[id_usuario]);
      verSaldo(usuario, id_usuario, &empleados[id_usuario],
               &clientes[id_usuario]);
      break;
    ...
    case 3: // Cambiar PIN actual
      cambiarPin(usuario, id_usuario, &empleados[id_usuario],
                 &clientes[id_usuario]);
      break;
    ...
    case 5:
      printf("\nGracias por utilizar nuestro cajero automatico, que tengas "
             "buen día!");
      looper = false;
      break;
...
void retirarDinero(int usuario, int id_usuario, float retiro,
  ...
  else if (retiro >= cliente[id_usuario].saldo)
    printf("Fondos insuficientes!");
  else
