...
Se requiere de un algoritmo para programar un sistema de máquina de refrescos.
En este prototipo existen 4 opciones de compra. 1: soda a $0.80 centavos. 2:
Jugos naturales a $1.00. 3: Agua embotellada a $1.25. 4: Malta a $0.75 centavos.
- La máquina acepta monedas de 5, 10, 25 y dólares de $1 y $5.
- En este prototipo se le pregunta al usuario qué opciones desea adquirir, el
usuario puede pedir de diferentes opciones y diferentes cantidades.
- El prototipo debe mostrar el monto a pagar y el usuario deberá proporcionar el
monto en cantidades dependiendo si es moneda o dólares.
- Finalmente, el prototipo deberá indicar al usuario si puede retirar el
producto, además, de dar el vuelto en monedas si lo amerita.
...
int articulo, x, y, vuelto1, vuelto2, vuelto3, vuelto4, vuelto5, sodas, jugos,
    agua, maltas, cantidadProducto, billete5, billete1, moneda1, moneda2,
    moneda3;
...
int main() {
  ...
  while (x == 1) {
    ...
    printf("\nIntroduzca la cantidad de billetes de 5$: ");
    scanf("%i", &billete5);
    printf("\nIntroduzca la cantidad de billetes de 1$: ");
    scanf("%i", &billete1);
    printf("\nIntroduzca la cantidad de monedas de 0.25: ");
    scanf("%i", &moneda1);
    printf("\nIntroduzca la cantidad de monedas de 0.10: ");
    scanf("%i", &moneda2);
    printf("\nIntroduzca la cantidad de monedas de 0.05: ");
    scanf("%i", &moneda3);
    pagado = (billete5 * 5) + (billete1 * 1) + (moneda1 * 0.25) +
             (moneda2 * 0.10) + (moneda3 * 0.05);
    if (montototal > pagado) {
      printf("\nEl dinero ingresado no es suficiente! Intente nuevamente, "
             "presione 1 para intentar nuevamente, 0 para salir: ");
      scanf("%i", &x);
      if (x == 0) {
        printf("Se le ha devuelto la cantidad ingresada que fue de: %.2f$",
               pagado);
      }
