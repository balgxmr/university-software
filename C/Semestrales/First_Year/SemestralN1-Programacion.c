...
// constantes globales
const char listaPlacasAutorizadas[15][10] = {
    "PAK798", "GKS682", "FRG681", "KGS563", "SGF952",
    "GSG295", "CK7193", "FT2553", "OT6559", "LS5463",
    "967AFW", "368SFW", "PR5467", "SG8786", "456BNM"};
const char listaNombresAutorizados[15][30] = {
    "Richard Herranz", "Jose Encalada",    "Francisco Perez",    "Maria Viñas",
    "Jesus Borras",    "Francisca Casals", "Gustavo Valenzuela", "Omar Vallejo",
    "Gaspar Andres",   "Neus Rus",         "Veronica Coca",      "Saul Roman",
    "Ana Carballo",    "Alejandra Otero",  "Florin Iglesias"};
...
int main() {
  // bloque de variables
  int operacion, x, numeroEstacionamiento, menu, indice, m, i,
      identificadorEstacionamientos, j, jose, richard, k,
      estacionamientosLLenos, lugar;
  ...
  while (menu == 0) {
    ...
    if (operacion <= 0 || operacion > 6) {
      system("cls");
      printf("\nSu opción no es válida! Ingrese una opción correcta, del 1 al "
             "6 como se muestra en el siguiente menú.\n\n");
    }
    switch (operacion) {
    case 1:
      system("cls");
      ...
      if (indice > 0) {
        ...
        for (i = 0; i < 10; i++) {
          if (estacionamientos[i] == indice) {
            printf("|                                                   Su "
                   "carro ya esta estacionado en el espacio numero %i.         "
                   "                                               |\n",
                   i + 1);
            printf("-----------------------------------------------------------"
                   "-----------------------------------------------------------"
                   "-------------------------------------------\n");
            k = 0;
            while (k == 0) // Inicio de la tabla para mostrar el espacio actual
                           // del propietario.
            {
              for (i = 0; i < 10; i++) {
                if (i == 5) {
                ...
                if (estacionamientos[i] == indice) {
                  ...
                  if (estacionamientos[i] != 0) {
                    ...
                    printf("|\t\033[32mEspacio %i disponible.\033[0m\t|",
                           i + 1);
                  }
                }
              }
            ...
            printf("-----------------------------------------------------------"
                   "-----------------------------------------------------------"
                   "-------------------------------------------\n");
            printf("\n"); // Fin de la tabla que muestra el espacio actual del
                          // propietario.
            printf(
                "Puede retirarlo ingresando a la opción numero 3 del menú.\n");
        ...
        if (identificadorEstacionamientos == -1) {
          ...
          printf("-------------------------------------------------------------"
                 ...
                 "---------------------------------------\n");
          printf("\n");
          estacionamientosLLenos =
              verificarEstacionamientosLlenos(estacionamientos);
          if (estacionamientosLLenos == 1) {
            x = 0;
            while (x == 0) {
              printf("Seleccione en cual espacio disponible desea estacionar "
                     "su vehiculo: ");
              scanf("%i", &numeroEstacionamiento);
              if ((numeroEstacionamiento < 1) || (numeroEstacionamiento > 10)) {
                printf("El estacionamiento %i no es valido! Seleccione un "
                       "estacionamiento de los disponibles.\n",
                ...
                x = 1;
                printf("\033[32mSu carro ha sido reservado en el espacio "
                       "%i.\033[0m\n",
                       numeroEstacionamiento);
              } else {
                printf("Ese estacionamiento esta actualmente ocupado, "
                       "seleccione otro. \n");
              }
            }
          } else {
            printf("Todos los estacionamientos estan ocupados actualmente, "
                   "porfavor regrese mas tarde. \n");
          }
        }

      } else {
        printf("Su placa no esta registrada en nuestro sistema, por lo tanto "
               "no tiene acceso al estacionamiento. \n");
      }
      volverYSalir();
      break;
    case 3:
      ...
      if (indice > 0) {
        ...
        if (m < 0) {
          printf("Usted aun no ha estacionado su carro, puede ir a la opcion "
                 "numero 2 para estacionarlo\n");
        }
      } else {
        printf("Su placa no esta registrada en nuestro sistema, por lo tanto "
               "no tiene acceso al estacionamiento");
      ...
      for (i = 0; i < 10; i++) {
        ...
        for (j = 0; j < 30; j++) {
          if (registro[i][j] == 0) {
            ...
            printf(" [ %s (%s) ]", listaNombresAutorizados[registro[i][j] - 1],
                   listaPlacasAutorizadas[registro[i][j] - 1]);
          }
        }
        printf("------------------\n");
      }
      ...
      while (richard < 16) {
        if (strlen(listaNombresAutorizados[jose]) < 15) {
          printf(
              "----------------------------------------------------------\n");
          printf("| %i)\t|%s\t\t|\t%s\t\t|\n", richard,
                 listaNombresAutorizados[jose], listaPlacasAutorizadas[jose]);
        } else {
          printf(
              "----------------------------------------------------------\n");
          printf("| %i)\t|%s\t|\t%s\t\t|\n", richard,
    ...
    case 6:
      ...
      printf("El programa ha finalizado. ¡Hasta pronto! La UTP le desea que "
             "tenga un buen día.\n\n");
      menu = 1;
      break;
    }
