...
int main() {
  ...
  printf("%i %f %c\n", a, b,
         c); // Debemos de asignar (a,b,c) en el mismo orden en que están los %
  ...
  scanf(
      "%i",
      &d); // scanf se utiliza para guardar datos. el "&d" debe estar
           // acompañado, este pide la variable d y guarda el valor en la misma.
  ...
  // scanf("%s",x); // OJO! El %s es porque son caracteres/strings, y el "&" no
  // hace falta. Además, se utiliza "s" para el tipo caracter.

  // OJOOOOOOOOO. EL SCAN SOLO LE HASTA DONDE HAY UN ESPACIO (Ejemplo:
  // introducimos José Antonio, en el output saldrá José), POR LO CUAL, ES MEJOR
  // UTILIZAR:
  gets(x);
