...
public class ProyectoERD {
  public static void main(String[] args) throws Exception {
    ...
    String mesesDelAnio[] = {
        ...
        "Noviembre", "Diciembre"}; // arreglo para imprimir los meses del año

    Estudiante estudiantes[] =
        new Estudiante[100]; // arreglo de objetos donde se almacenara en cada
                             // posicion todos los atributos del registro

    BufferedReader leer = new BufferedReader(
        new InputStreamReader(System.in)); // para leer los valores e imprimir
                                           // en pantalla
    ...
    for (int i = 0; i < estudiantes.length; i++) {
      ...
      boolean loop = true;
      while (loop) { // ciclo while para que el usuario ingrese un valor
                     // aceptable en el indice dentro del rango 0 a 3
      ...
      estudiantes[i] = new Estudiante(nombre, cedula, mes, año, carrera,
                                      indice); // se envian los datos como
    ...
    for (int i = 0; i < estudiantes.length; i++) {
      for (int j = 0; j < estudiantes.length - i - 1; j++) {
        ...
        if (estudiantes[j + 1].getCarrera().compareTo(
                estudiantes[j].getCarrera()) < 0 ||
            (estudiantes[j + 1].getCarrera().equals(
                 estudiantes[j].getCarrera()) &&
    ...
    // algoritmo para imprimir ordenadamente por carrera 5 personas con el
    // indice mas alto, tambien valida si el indice es mayor a 2.5
    for (int i = 0; i < estudiantes.length; i++) {
      ...
      if (estudiantes[i].getIndice() >= 2.5 && n < 5) {
        System.out.println("Nombre: " + estudiantes[i].getNombre() +
                           " | Mes de nacimiento: " +
                           mesesDelAnio[estudiantes[i].getMes() - 1] +
                           " | Carrera: " + estudiantes[i].getCarrera() +
                           " | Indice: " + estudiantes[i].getIndice());
        n += 1;
      }
    } // fin for
  }   // fin main
