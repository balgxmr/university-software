  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      for (int i = 2; i <= numero;
           i++) { // Este bucle itera sobre todos los números enteros desde 2
                  // hasta el número dado.
        boolean validarNumPrimo =
            true; // Usado para saber si el numero es primo o no. Se inicializa
                  // al comienzo del 1er for.
        for (int j = 2; j <= i / 2;
             j++) { // Ejecuta desde 2 hasta la mitad de N, comprueba si "i" es
                    // divisible entre 2 o su mitad
          if (i % j == 0) { // Si es divisible, "i" no es primo, procede a:
            validarNumPrimo =
                false; // validarNumPrimo = verdadero (rompe ciclo)
            break;
          }
        } // j for end

        if (validarNumPrimo) { // Si validarNumPrimo = true, imprime el numero
                               // primo
          System.out.print(i + ", ");
        } // if end
      }   // i for end
